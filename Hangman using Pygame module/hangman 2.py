...
def redraw_game_window():
    ...
    for i in range(len(buttons)):
        if buttons[i][4]:
            ...
            win.blit(label, (buttons[i][1] - (label.get_width()), buttons[i][2] - (label.get_height()))) #printing the letter into the screen
    ...
    pic = hangmanPics[limbs]  #loading each pic depending upon the limb
...
def randomWord():  #function for choosing any word from word.txt
    file = open('words.txt') #Automatically in reading mode
    f = file.readlines()
